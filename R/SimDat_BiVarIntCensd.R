#' @title Simulate Bivariate Interval-censored Data
...
#' @param case Either one of \code{c(1,2)}, where 1 for current status data and 2 for interval-censored data. Default is \code{case = 2}
...
SimDat_BiVarIntCensd <- function(size, case = 2, lambda1, lambda2, K.tau, T1.start, T1.end, T2.start, T2.end, seed = 123){
  ...
  if (case == 1) { # current status data
    obs.T1 = runif(size, T1.start, T1.end)
    obs.T2 = runif(size, T2.start, T2.end)
    UV_1   = data.frame(tu_1 = ifelse(T1<obs.T1, 0, obs.T1), tv_1 = ifelse(T1<obs.T1, obs.T1, Inf), ind_1 = ifelse(T1<obs.T1, "Left", "Right"))
    UV_2   = data.frame(tu_2 = ifelse(T2<obs.T2, 0, obs.T2), tv_2 = ifelse(T2<obs.T2, obs.T2, Inf), ind_2 = ifelse(T2<obs.T2, "Left", "Right"))

  } else if (case == 2) { # interval censored data
    ...
    stop("The `case` is not properly setup! Please check.")
  }
